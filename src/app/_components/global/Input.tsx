...
export function TextField({ label, placeholder, className, name, required, type, handleChange, value, onKeyDown, disabled, variant }: Readonly<TextFieldProps>) {
  const [showPassword, setShowPassword] = useState(false);
  return (
    <div className={clsx("flex flex-col my-6 gap-2", className)}>
      {label && (
        <label htmlFor={name} className={clsx(`first-letter:capitalize ${required ? "after:text-red-500 after:content-['*']" : ""}`)}>
          {label}
        </label>
      )}
      <div className="relative">
        {type == "password" && (
          <button className="absolute right-3 mt-4 flex items-center px-2 text-neutral-400 hover:text-neutral-500 transition-all" type="button" onClick={() => setShowPassword(!showPassword)}>
...
export function TextArea({ label, placeholder, className, required, name, value, disabled }: Readonly<InputProps>) {
  return (
...
export function SelectField({ label, options, className, required, value, name, handleChange, disabled }: Readonly<SelectFieldProps>) {
  return (
    <div className={"flex flex-col gap-2 my-6 " + className}>
      {label && (
        <label htmlFor={name} className={clsx(`first-letter:capitalize ${required ? "after:text-red-500 after:content-['*']" : ""}`)}>
